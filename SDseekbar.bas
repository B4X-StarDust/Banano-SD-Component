...
#Event: Change (Percent As Double)
...
#DesignerProperty: Key: mValue, DisplayName: Initial %, FieldType: Int, DefaultValue: 0, MinRange: 0, MaxRange: 100,Description: Initial percentage (0..100)
...
Public Sub DesignerCreateView (Target As BANanoElement, Props As Map)
	...
	mElement = mTarget.Append($"<div id="${mName}" class="${mClasses}" style="${exStyle}${mStyle}">
	  <div class="sdseekbar" id="${mName}-seekbar">
	    <div class="sdseekbar-progress" id="${mName}-Progress"></div>
	    <div class="sdseekbar-thumb" id="${mName}-Thumb"></div>
	  </div>
	</div>"$).Get("#" & mName)
	...
	BANano.GetElement($"#${mName}-seekbar"$).HandleEvents("click",Me,"update_seekbar")
	BANano.GetElement($"#${mName}-seekbar"$).HandleEvents("mousemove",Me,"drag_seekbar")
	setValue(mValue)
End Sub

Private Sub update_seekbar (event As BANanoEvent) 
	Dim seekbar As BANanoElement = BANano.GetElement($"#${mName}-seekbar"$)
	Dim seekbarProgress As BANanoElement = BANano.GetElement($"#${mName}-Progress"$)
	Dim seekbarThumb As BANanoElement = BANano.GetElement($"#${mName}-Thumb"$)
		
	percent = Max(Min(((event.ClientX - seekbar.OffsetLeft) / seekbar.OffsetWidth) * 100,100),0)

	seekbarProgress.SetStyle($" {"width":"${percent}%"} "$)
	seekbarThumb.SetStyle($" {"left":"${percent}%"} "$)
		
	BANano.CallSub(mCallBack,mEventName & "_change",Array (percent))
End Sub

Private Sub drag_seekbar (event As BANanoEvent)
	If event.Buttons=1 Then update_seekbar(event)
End Sub
...
Public Sub setValue (v As Double)
	Dim seekbarProgress As BANanoElement = BANano.GetElement($"#${mName}-Progress"$)
	Dim seekbarThumb As BANanoElement = BANano.GetElement($"#${mName}-Thumb"$)
	percent= Max(0,Min(v,100))

...
#Event: Click (Item As String, ID As String)
...
#DesignerProperty: Key: mItem, DisplayName: Item, FieldType: String, DefaultValue: Item 1;Item 2;Item 3, Description: item separated by ;
#DesignerProperty: Key: midItem, DisplayName: ID Item, FieldType: String, DefaultValue: It1;It2;It3, Description: id item separated by ;
...
Public Sub DesignerCreateView (Target As BANanoElement, Props As Map)
	...
	If Props <> Null Then
		mItem = Props.Get("mItem")
		midItem	= Props.Get("midItem")
	...
	mElement = mTarget.Append($"<div id="div_${mName}" class="${mClasses}" style="${exStyle}${mStyle}"></div>"$).Get("#div_" & mName)
	...
	Dim itm() As String = SDfunc.SDRegEx(";",mItem)
	Dim iditm() As String = SDfunc.SDRegEx(";",midItem)
	Changeitem(itm,iditm)
End Sub

Public Sub Changeitem(Items() As String, idItems() As String)
	Dim ContentOption As String =$"<ul class="sdlistview" id="ul-${mName}">"$
	Dim lop As Int = Min(idItems.Length,Items.Length)
	
	For i=0 To lop-1
		ContentOption=$"${ContentOption}<li id="li-${mName}_${idItems(i)}" Data="${Items(i)}" ">${Items(i)}</li>"$
	Next
	ContentOption=ContentOption & "</ul>"
	
	mElement.Empty
	mElement.Append(ContentOption)
	
	For i=0 To lop-1
		BANano.GetElement($"#li-${mName}_${idItems(i)}"$).HandleEvents("click", Me, "Event_Click")
	Next
End Sub

Private Sub Event_Click (event As BANanoEvent)
	Dim Id() As String =  SDfunc.SDRegEx("_",event.id)
	Dim Item As String = BANano.GetElement($"#li-${mName}_${Id(1)}"$).GetText
	'event.Value
	BANano.CallSub(mCallBack, mEventName & "_Click",Array As String(Item,Id(1)))
End Sub
...
Public Sub setBackgroundColor(Color As String)
	BANano.GetElement($"#ul-${mName}"$).SetStyle($"{"background-color":"${Color}"}"$)
End Sub


Public Sub setTextColor(Color As String)
	BANano.GetElement($"#ul-${mName}"$).SetStyle($"{"color":"${Color}"}"$)
End Sub

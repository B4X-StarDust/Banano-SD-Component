...
#DesignerProperty: Key: nMessage, DisplayName: Message, FieldType: String, DefaultValue: Select date, Description: Time Picker Message
#DesignerProperty: Key: mWidth, DisplayName: Width, FieldType: String, DefaultValue: 200px, Description: Width (px or %) of label - none for automatic
#DesignerProperty: Key: Classes, DisplayName: Classes, FieldType: String, DefaultValue: sddate-picker, Description: Classes added to the HTML tag.
...
Public Sub DesignerCreateView (Target As BANanoElement, Props As Map)
	...
	Dim exStyle As String = $"display: flex; justify-content: center;align-items: center; "$ & BuildExStyle
	
	mElement = mTarget.Append($"<div id="${mName}" class="${mClasses}" style="${exStyle}${mStyle}">
	<div style="width: ${mWidth};box-sizing: border-box;">
	<div id="${mName}_label" style="padding-top: 5px;">${nMessage}</div>
    <input type="date" id="${mName}_date" class="${mClasses}" /></div>
	</div>"$).Get("#" & mName)
	...
	mTarget.Get($"#${mName}_date"$).HandleEvents("change", mCallBack, mEventName & "_click")
	Dim Now As Long = DateTime.now
	SetDate(DateTime.GetDayOfMonth(Now),DateTime.GetMonth(Now),DateTime.GetYear(Now))
End Sub

Public Sub GetDate As String
	Return mTarget.Get($"#${mName}_date"$).GetValue
End Sub

' tp.time="18:00"
Public Sub SetDate(Day As Int, Month As Int, Year As Int)
	mTarget.Get($"#${mName}_date"$).setValue($"${Year}-${NumberFormat(Month,2,0)}-${NumberFormat(Day,2,0)}"$)
End Sub

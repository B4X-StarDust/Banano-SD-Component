...
#DesignerProperty: Key: nColsS, DisplayName: Cols(1/6) Sm, FieldType: int, DefaultValue: 1, MinRange: 1, MaxRange: 12, Description: N. of Columns - Small Display
#DesignerProperty: Key: nColsM, DisplayName: Cols(1/12) Md, FieldType: int, DefaultValue: 1, MinRange: 1, MaxRange: 12, Description: N. of Columns - Medium Display
#DesignerProperty: Key: nColsL, DisplayName: Cols(1/12) Lg, FieldType: int, DefaultValue: 1, MinRange: 1, MaxRange: 12, Description: N. of Columns - Large Display
...
#DesignerProperty: Key: textalign, DisplayName: Align, FieldType: String, DefaultValue: center, List: none|center|left|right, Description: Align
...
Sub Class_Globals
	...
	Private mBackground As String = "" 'ignore
	Private nColsS As Int = 0
	Private nColsM As Int = 0
	Private nColsL As Int = 0
...
Public Sub DesignerCreateView (Target As BANanoElement, Props As Map)
	...
	If Props <> Null Then
		...
		nColsS = 2 * 8.33 * Props.Get("nColsS")
		nColsM = 8.33 * Props.Get("nColsM")
		nColsL = 8.33 * Props.Get("nColsL")
		mAlign = Props.Get("textalign")
	...
	If mAlign="none" Then mAlign="" Else mAlign=$"text-align: ${mAlign};"$
	If BANano.Window.InnerWidth<768 Then 
		perc = NumberFormat(nColsS,1,1)
	Else if BANano.Window.InnerWidth<1024 Then 
		perc = NumberFormat(nColsM,1,1)
	Else 
		perc = NumberFormat(nColsL,1,1)
	End If
	perc=perc.Replace(",",".")
	
	Dim exStyle As String = $"width: ${perc}%; margin: 0 auto; box-sizing: border-box; overflow: hidden; ${BuildExStyle}"$
	mElement = mTarget.Append($"<div id="${mName}" class="${mClasses}" style="${mAlign} ${exStyle}${mStyle}"></div>"$).Get("#" & mName)
	'mElement = mTarget.Append($"<div id="${mName}" class="${mClasses}" style="max-width: 16.34%;${exStyle}${mStyle}"></div>"$).Get("#" & mName)

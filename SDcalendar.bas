...
Version=10
@EndOfDesignText@
#Event: DayClick (Day As Int)
#Event: ChangeCalendar (Year As Int, Month As Int)
...
#DesignerProperty: Key: mWidth, DisplayName: Width, FieldType: String, DefaultValue: , Description: Width (px or %) of label - none for automatic
#DesignerProperty: Key: mBackground, DisplayName: Background Color, FieldType: Color, DefaultValue: 0xFF4CAF50, Description: Title Background Color
...
Sub Class_Globals
	...
	Private tDay As Int
	Private mMese As Int
	Private mAnno As Int
...
Public Sub Initialize (CallBack As Object, Name As String, EventName As String)
	...
	tDay=0
	monthName = Array As String("Gennaio", "Febbraio", "Marzo", "Aprile", "Maggio", "Giugno", "Luglio", "Agosto", "Settembre", "Ottobre", "Novembre", "Dicembre")
	dayName = Array As String("Lun","Mar","Mer","Gio","Ven","Sab","Dom")
End Sub
...
Public Sub DesignerCreateView (Target As BANanoElement, Props As Map)
	...
	If mWidth<>"" Then exStyle=$"width: ${mWidth};${exStyle}"$
	mElement = mTarget.Append($"<div id="${mName}" class="sdcalendar" style="${exStyle}${mStyle}">
	${Generate(DateTime.GetMonth(DateTime.Now),DateTime.GetYear(DateTime.Now))}</div>"$).Get("#" & mName)
...
Private Sub Generate(month As Int , year As Int) As String
	Dim html As String = ""
	Dim FirstDay As Int = DateTime.DateParse($"${NumberFormat(month,2,0)}/01/${year}"$)
	Dim Dow As Int = (DateTime.GetDayOfWeek(FirstDay) + 5) Mod 7
	tDay = DaysInMonth(month,year)
	Dim DayEmpty As String = ""
	Dim DayFull As String = ""
	
	For i=0 To Dow-1
		DayEmpty = $"${DayEmpty}<div class="sdday empty"></div>"$
	Next
	For d=1 To tDay
		DayFull = $"${DayFull}<div class="sdday" id="day_${mName}_${d}">${d}</div>"$
	Next
	
	html = $"<div class="sdcalendar-header" style="background-color: ${mBackground};" >
		<button id="sx_${mName}" class="sdnav-button" onclick="prevMonth()">‹</button>
		<div id="ms_${mName}" class="sdmonth-title">${monthName(month-1)} ${year}</div>
		<button id="dx_${mName}" class="sdnav-button" onclick="nextMonth()">›</button>
	</div>
	<div class="sdcalendar-body">
		<!-- Intestazione dei giorni della settimana -->
		<div class="sdday sdday-header">${dayName(0)}</div>
		<div class="sdday sdday-header">${dayName(1)}</div>
		<div class="sdday sdday-header">${dayName(2)}</div>
		<div class="sdday sdday-header">${dayName(3)}</div>
		<div class="sdday sdday-header">${dayName(4)}</div>
		<div class="sdday sdday-header">${dayName(5)}</div>
		<div class="sdday sdday-header">${dayName(6)}</div>
...
Private Sub CreateEvent
	BANano.GetElement($"#sx_${mName}"$).HandleEvents("click", Me, "dietro_click")
	BANano.GetElement($"#dx_${mName}"$).HandleEvents("click", Me, "avanti_click")
	
	For d=1 To tDay
		BANano.GetElement($"#day_${mName}_${d}"$).HandleEvents("click", Me, "day_click")
	Next
End Sub

Private Sub day_click (event As BANanoEvent)
	Dim ID As String = event.ID
	Dim Day As Int = BANano.GetElement($"#${ID}"$).GetText

	BANano.CallSub(mCallBack,mEventName & "_DayClick",Array (Day))
End Sub
...
Private Sub dietro_click
	...
	If mMese=0 Then 
		...
		mAnno=mAnno-1
	End If
	BANano.GetElement($"#${mName}"$).Empty
	BANano.GetElement($"#${mName}"$).Append(Generate(mMese,mAnno))
	CreateEvent
	BANano.CallSub(mCallBack,mEventName & "_ChangeCalendar",Array (mAnno,mMese))
End Sub
...
Private Sub DaysInMonth(Month As Int, year As Int) As Int
	Month=Month+1
	If Month>12 Then 
		Month=1
		year=year+1
	End If
	Dim l As Long = DateTime.DateParse($"${Month}/01/${year}"$)
	l=l-DateTime.TicksPerDay
	Return DateTime.GetDayOfMonth(l)
End Sub
...
' Cal.SetDay(10,"#4CAF50")
Public Sub SetDay(nDay As Int,TextColor As String , color As String)
	Dim D As BANanoElement = BANano.GetElement($"#day_${mName}_${nDay}"$)
	D.SetStyle($"{ "background-color": "${color}","color":"${TextColor}", "border-radius": "20px"}"$)
End Sub

Public Sub SetSelectDay(nDay As Int,TextColor As String , color As String)
	Dim D As BANanoElement = BANano.GetElement($"#day_${mName}_${nDay}"$)
	D.SetStyle($"{ "background-color": "${color}","color":"${TextColor}"}"$)
End Sub

Public Sub ResetDay(nDay As Int)
	Dim D As BANanoElement = BANano.GetElement($"#day_${mName}_${nDay}"$)
	D.SetStyle($"{ "background-color": "#f9f9f9","color":"black", "border-radius": "0px"}"$)
End Sub

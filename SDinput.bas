...
#DesignerProperty: Key: nMessage, DisplayName: Message, FieldType: String, DefaultValue: , Description: Input element
#DesignerProperty: Key: nName, DisplayName: Name, FieldType: String, DefaultValue: MyInput, Description: Namefor form
#DesignerProperty: Key: mType, DisplayName: Type input, FieldType: String, DefaultValue: , List: text|password|email|number|tel|url|date|time|month|year|week|checkbox|radio|color|file,Description: Classes added to the HTML tag.
#DesignerProperty: Key: mFontSize, DisplayName: Font Size, FieldType: Int, DefaultValue: 14, Description: Font Size
#DesignerProperty: Key: mWidth, DisplayName: Width, FieldType: String, DefaultValue: , Description: width
#DesignerProperty: Key: mHeight, DisplayName: Height, FieldType: String, DefaultValue: , Description: height
#DesignerProperty: Key: mBorder, DisplayName: Border, FieldType: Int, DefaultValue: 2, MinRange: 0, MaxRange: 5,Description: Border
#DesignerProperty: Key: Classes, DisplayName: Classes, FieldType: String, DefaultValue: sdinput-picker, Description: Classes added to the HTML tag.
...
Sub Class_Globals
	...
	Private nMessage As String = ""
	Private nName As String = ""
	Private mType As String = ""
	Private mBorder As Int = 1
...
Public Sub DesignerCreateView (Target As BANanoElement, Props As Map)
	...
	If Props <> Null Then
		nMessage = Props.Get("nMessage")
		nName = Props.Get("nName")
	...
	'placeholder="Hint"
	mElement = mTarget.Append($"<div id="${mName}" class="${mClasses}" style="${exStyle}">
	<div style="padding:8px; min-height: 30px;box-sizing: border-box;">
	<input type="${mType}" id="${mName}_input" name="${nName}" style="width: 100%;height: ${mHeight};font-size: ${mFontSize}px;${mStyle}" />
	<label>${nMessage}</label><div>
	</div>"$).Get("#" & mName)
...
Public Sub getValue As String
	Return mTarget.Get($"#${mName}_input"$).GetValue
End Sub

' time="18:00"
' date="2025-12-12
Public Sub setValue(value As String)
	mTarget.Get($"#${mName}_input"$).setValue(value)
End Sub

Public Sub GetChecked As Boolean
	Return mTarget.Get($"#${mName}_input"$).GetChecked
End Sub

Public Sub SetChecked(value As Boolean)
	mTarget.Get($"#${mName}_date"$).SetChecked(value)
End Sub
...
public Sub BuildExStyle() As String
	...
	sb.Append($"border: ${mBorder}px solid #ccc; border-radius: 15px; margin-top: 5px;"$)
	If mWidth <> "" Then sb.Append("width: " & mWidth & ";")
	If mHeight <> "" Then sb.Append("height: " & mHeight & ";")

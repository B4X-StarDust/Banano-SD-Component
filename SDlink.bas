...
#DesignerProperty: Key: aID, DisplayName: automatic ID, FieldType: Boolean, DefaultValue: False, Description: automatic ID
#DesignerProperty: Key: Text, DisplayName: Text, FieldType: String, DefaultValue: , Description: Text of label
#DesignerProperty: Key: TextColor, DisplayName: Text Color, FieldType: Color, DefaultValue: 0xFF000000, Description: Text Color
#DesignerProperty: Key: Background, DisplayName: Background Color, FieldType: Color, DefaultValue: 0xFFFFFFFF, Description: Background Color
#DesignerProperty: Key: mFontSize, DisplayName: Font Size, FieldType: Int, DefaultValue: 16, Description: Font Size
#DesignerProperty: Key: mFontBold, DisplayName: Font Bold, FieldType: Boolean, DefaultValue: False, Description: Font bold
#DesignerProperty: Key: Source, DisplayName: img, FieldType: String, DefaultValue: ./assets/, Description: Source image added to the HTML tag.
#DesignerProperty: Key: sWidth, DisplayName: img Width, FieldType: String, DefaultValue: , Description: Width (px or %) of image - none for automatic
#DesignerProperty: Key: sHeight, DisplayName: img Height, FieldType: String, DefaultValue: , Description: Height (px or %) of image - none for automatic
#DesignerProperty: Key: URL, DisplayName: link url, FieldType: String, DefaultValue: https://www.b4x.com/, Description: Source url to navigate
#DesignerProperty: Key: mWidth, DisplayName: Width, FieldType: String, DefaultValue: , Description: Width (px or %) of container - none for automatic
#DesignerProperty: Key: mHeight, DisplayName: Height, FieldType: String, DefaultValue: , Description: Height (px or %) of container - none for automatic
#DesignerProperty: Key: mAlign, DisplayName: Align H, FieldType: String, DefaultValue: center, List: none|center|left|right, Description: Align Horizontal
#DesignerProperty: Key: mAlignVert, DisplayName: Align V, FieldType: String, DefaultValue: center, List: flex-start|flex-end|center|baseline|stretch, Description: Align Vertical
#DesignerProperty: Key: Classes, DisplayName: Classes, FieldType: String, DefaultValue: sd-label, Description: Classes added to the HTML tag.
...
Public Sub DesignerCreateView (Target As BANanoElement, Props As Map)
	...
	If Props <> Null Then
		aID = Props.GetDefault("aID",True)
		Source = Props.Get("Source")
		mURL = Props.Get("URL")
		mText = Props.Get("Text")
		mTextColor = Props.Get("TextColor")
		mBackground = Props.Get("Background")
		mFontSize = Props.Get("mFontSize")
		mFontBold = Props.Get("mFontBold")
		sHeight = Props.Get("sHeight")
		sWidth = Props.Get("sWidth")
		mHeight = Props.Get("mHeight")
		mWidth = Props.Get("mWidth")
		mAlign = Props.Get("mAlign")
		mAlignVert = Props.Get("mAlignVert")
	...
	If aID Then mName=mName & Rnd(100000,999999)

	Dim exStyle As String = BuildExStyle
	If Source.Replace("./assets/","").Trim<>"" Then img=$"<img src="${Source}" style="width:${sWidth};height:${sHeight}" alt="link">"$
	mElement = mTarget.Append($"<div id="div_${mName}" class="${mClasses}" style="${exStyle}${mStyle}">${img}${mText}</div>"$).Get("#div_" & mName)
	
	'' defining events is very simple. Note that it has to be run AFTER adding it to the HTML DOM! eventName must be lowercase!
	mElement.HandleEvents("click", Me, "here_click")
	'mElement.HandleEvents("mouseenter", Me, "mouseenter")
	setTextColor(mTextColor)
	setBackgroundColor(mBackground)
End Sub

Private Sub here_click
	Dim options As String
	options = "width=1024,height=768,top=100,left=100,scrollbars=yes,resizable=yes"
	BANano.Window.Open(mURL, "_blank", options)
End Sub
...
Public Sub setBackgroundColor(Color As String)
	...
	mElement.SetStyle($"{"background-color":"${Color}"}"$)
End Sub
...
public Sub BuildExStyle() As String
	...
	sb.Initialize
	If mBackground <> "" Then sb.Append("background-color: " & mBackground & ";")
	If mTextColor <> "" Then sb.Append("color: " & mTextColor & ";")
	...
	If mFontSize <> "" Then sb.Append("font-size: " & mFontSize & "px;")
	If mFontBold  Then sb.Append("font-weight: bold;")
	If mHeight<>"" Then sb.Append("max-height: " & mHeight & ";")
	If mWidth<>"" Then sb.Append("width: " & mWidth & ";")
	If mAlign<>"none" Then sb.Append("text-align: " & mAlign & ";")
	If mAlignVert<>"none" Then sb.Append("align-items: " & mAlignVert & ";")
	Return sb.ToString
End Sub
...
Public Sub label As BANanoElement
	Return BANano.GetElement("#" & mName)
End Sub

...
#DesignerProperty: Key: Text, DisplayName: Icon Name, FieldType: String, DefaultValue: fa-swimmer, Description: https://fontawesome.com/icons/
#DesignerProperty: Key: TextColor, DisplayName: Text Color, FieldType: Color, DefaultValue: 0xFFFFFFFF, Description: Text Color
#DesignerProperty: Key: Background, DisplayName: Background Color, FieldType: Color, DefaultValue: 0xFFF08080, Description: Background Color
#DesignerProperty: Key: mSize, DisplayName: Icon Size, FieldType: String, DefaultValue: 25px, Description: Icon size width/height
#DesignerProperty: Key: mAbsolute, DisplayName: Visible, FieldType: Boolean, DefaultValue: False, Description: Absolute position top, left
#DesignerProperty: Key: mTop, DisplayName: Margin Top, FieldType: String, DefaultValue: 15px, Description: Top for absolute
#DesignerProperty: Key: mLeft, DisplayName: Margin Left, FieldType: String, DefaultValue: 15px, Description: Left for absolute 
#DesignerProperty: Key: Classes, DisplayName: Classes, FieldType: String, DefaultValue: sdicon-button, Description: Classes added to the HTML tag.
...
Sub Class_Globals
	...
	Private mLeft As Int = 0
	Private mTop As Int = 0
...
Public Sub DesignerCreateView (Target As BANanoElement, Props As Map)
	...
	If Props <> Null Then
		...
		mSize = Props.Get("mSize")
		mAbsolute = Props.Get("mAbsolute")
		mTop = Props.Get("mTop")
		mLeft = Props.Get("mLeft")
	...
	If mAbsolute Then exStyle=$"position: absolute; top: ${mTop}; left: ${mLeft}; ${exStyle}"$
		
	mElement = mTarget.Append($"<div id="div_${mName}" class="" style="z-index: 1;${exStyle}${mStyle}">
	<button id="${mName}" Type="button" style="font-size: ${(mSize)};width: ${mSize*1.5}px; height: ${mSize*1.5}px;" class="${mClasses}"><i class="fas ${mText}"></i></button>
	</div>"$).Get("#" & mName)

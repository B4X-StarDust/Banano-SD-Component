...
#DesignerProperty: Key: mPlate, DisplayName: Plate, FieldType: String, DefaultValue: AB 123 CD, Description: Plate car
#DesignerProperty: Key: Classes, DisplayName: Classes, FieldType: String, DefaultValue: sd-targa, Description: Classes added to the HTML tag.
...
Public Sub DesignerCreateView (Target As BANanoElement, Props As Map)
	...
	Dim exStyle As String = "width: 99.1%;" & BuildExStyle
	
	mElement = mTarget.Append($"<div id="${mName}" class="${mClasses}" style="${exStyle}${mStyle}">
		<div class="blu" >I</div>
		<div class="testo" id="plate_${mName}">${mPlate}</div>
		<div class="blu" style="font-size: 18px;">EU</div>
	</div>"$).Get("#" & mName)
...
Public Sub setPlateCar(v As String)
	mPlate=v
	BANano.GetElement($"#plate_${mName}"$).SetText(v) 
End Sub

Public Sub getPlateCar As String
	Return mPlate
End Sub

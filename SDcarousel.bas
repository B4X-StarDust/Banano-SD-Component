...
#DesignerProperty: Key: mTimer, DisplayName: Timer, FieldType: Int, DefaultValue: 3000, Description: Timer of slide (0 for none)
...
Public Sub DesignerCreateView (Target As BANanoElement, Props As Map)
	...
	<div id="${mName}_inner" style="display: flex;transition: transform 0.5s ease-in-out;"></div>
	</div>"$).Get("#" & mName)
	...
	Popolate
	Dim cb As BANanoObject = BANano.callback(Me,"whe_el", Null)
	If mTimer>0 Then BANano.Window.SetInterval(cb,mTimer)
End Sub

Private Sub Popolate
	Dim inner As BANanoElement = BANano.GetElement($"#${mName}_inner"$)
	inner.Empty
	
	For i=0 To imgList.Size-1
		Dim item As String = $"<img style="width: 100%;display: block;" src="${imgList.Get(i)}" alt="Slide ${i}">"$
		inner.Append(item)
	Next
End Sub
...
Private Sub whe_el
	If imgList.Size>0 Then 
		currentIndex = (currentIndex + 1) Mod imgList.Size
		Dim inner As BANanoElement = BANano.GetElement($"#${mName}_inner"$)
		inner.SetStyle($" {"transform": "translateX(-${currentIndex * 100}%)"} "$)
	End If
End Sub
...
Public Sub AddImage(imgUrl As String)
	imgList.Add(imgUrl)
	BANano.Console.Log("img: " & imgList.size)
	Popolate
End Sub
...
public Sub BuildExStyle() As String
	...
	sb.Initialize
	sb.Append($"width: 100%;overflow: hidden;position: relative;"$)

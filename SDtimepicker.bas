...
#DesignerProperty: Key: nMessage, DisplayName: Message, FieldType: String, DefaultValue: Select time, Description: Time Picker Message
...
#DesignerProperty: Key: Classes, DisplayName: Classes, FieldType: String, DefaultValue: sdtime-picker, Description: Classes added to the HTML tag.
...
Public Sub DesignerCreateView (Target As BANanoElement, Props As Map)
	...
	<div id="${mName}_label" style="text-align: center; margin-top: 5px;">${nMessage}</div>
	<input type="time" id="${mName}_time" name="time"/></div>
	</div>"$).Get("#" & mName)
		
	'' defining events is very simple. Note that it has to be run AFTER adding it to the HTML DOM! eventName must be lowercase!
	mTarget.Get($"#${mName}_time"$).HandleEvents("change", mCallBack, mEventName & "_click")
	'mElement.HandleEvents("mouseenter", Me, "mouseenter")
	Dim Now As Long = DateTime.now
	SetTime(DateTime.GetHour(Now),DateTime.GetMinute(Now))
End Sub

Public Sub GetTime As String
	Return mTarget.Get($"#${mName}_time"$).GetValue
End Sub

' tp.time="18:00"
Public Sub SetTime(Hours As Int, Minutes As Int)
	mTarget.Get($"#${mName}_time"$).setValue($"${NumberFormat(Hours,2,0)}:${NumberFormat(Minutes,2,0)}"$)
End Sub

...
#DesignerProperty: Key: nowrap, DisplayName: No wrap, FieldType: boolean, DefaultValue: False, Description: Arranged on a single line it does not wrap
...
#DesignerProperty: Key: mHeight, DisplayName: Height, FieldType: String, DefaultValue: , Description: Height  (px or %) of row
...
Sub Class_Globals
	...
	Private mnowrap As Boolean = False
...
public Sub BuildExStyle() As String
	...
	sb.Initialize
	sb.Append($"width:100%; display: flex; gap: 5px;"$)
	...
	If mAlignVert<>"none" Then sb.Append("align-items: " & mAlignVert & ";text-align: " & mAlignVert & ";")
	If mHeight<>"" Then sb.Append($"height: ${mHeight};"$)
	If mnowrap Then sb.Append("white-spacet: nowrap;")

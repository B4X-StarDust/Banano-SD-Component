...
#DesignerProperty: Key: Classes, DisplayName: Classes, FieldType: String, DefaultValue: sdclock, Description: Classes added to the HTML tag.
...
'	mElement = mTarget.Append($"<div id="${mName}" style="${exStyle}${mStyle}">00:00:00
'    <script>
...
'        document.getElementById('${mName}').firstChild.nodeValue = timeString;
...
'    </script>
	...
	mElement = mTarget.Append($"<div id="${mName}" class="${mClasses}" style="${exStyle}${mStyle}">00:00:00</div>"$).Get("#" & mName)
...
Private Sub refresh
	Do While True		
		mElement.SetText(DateTime.Time(DateTime.Now))
		Sleep(1000)
	Loop
End Sub
